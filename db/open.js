...
function draw() {
  ...
  background(14, 18, 24) // Dark background
  ...
  Object.entries(signalObj).forEach(([rawNetId, entry]) => {
    ...
    tokens.forEach(token => {
      ...
      if (ref && partPinMap[ref.partId] && partPinMap[ref.partId][ref.pinId]) {
        ...
        pinObj.netId = netId // Assign Net ID (Resolved Name) to Pin
    ...
    if (netPins.length > 0) {
      ...
      // Note: simple push might add duplicates if the same pin is referenced in multiple segments, 
      // but usually a pin is in one segment. If segments are split, they are different entries.
      // We'll just push for now.
    ...
    if (polyline.length >= 2) {
      ...
      polyline.netId = netId // Assign Net ID to Polyline for trace highlighting
...
function drawLayer_BoardLines(scaleFactor) {
  // No Culling as requested by user
  // Batch rendering optimization: Reduce state changes and matrix ops
      ...
      if (shapeKind === 'rect') {
         ...
         if (isSemi) {
           if (boardFilters.semiAsOutlines) {
             push()
             stroke(180, 180, 180, 220)   //color outlines component
...
function drawLayer_Signals(scaleFactor) {
  ...
  stroke(100, 255, 100, 80) // Greenish faint traces
...
function drawLayer_VirtualOutlines(scaleFactor) {
  const vb = getVisibleBounds()
  stroke(80, 255, 120, 220)
  noFill()
  const weight = 1 / scaleFactor
  strokeWeight(weight)
  renderData.virtualOutlines.forEach(v => {
    // Culling (Approximate)
    // v.center is local. v.size is local dimensions.
    // Calculate max extent from base_x/y
    ...
    rectMode(CENTER)
    rect(v.center.x, v.center.y, v.size.w, v.size.h, 0)
    pop()
  })
}
  ...
  renderData.pins.forEach(pin => {
    ...
    if (pin.netId) {
      if (pin.netId === 'GND') {
        ...
        fill(180, 180, 180, 200) // Light Gray
      }
    }
...
function drawLayer_ComponentLabels(scaleFactor) {
  const vb = getVisibleBounds()
  // Fixed screen size for text (e.g. 10px)
  // To achieve fixed screen size S, we use textSize(S / scaleFactor)
  ...
  // Collision Buffer in World Coordinates
  // We'll store bounding boxes [minX, minY, maxX, maxY]
  const occupied = []
  const PADDING = 2 / scaleFactor 
  
  // We iterate through components. Since the order is fixed, 
  // the ones that appear first get priority.
  // If we want "important" components to show up first, we'd need to sort.
  // For now, let's just use the array order.
  
  // Create a map for fast lookup of virtual outlines by ID
  const virtualMap = {}
  renderData.virtualOutlines.forEach(v => {
  ...
  renderData.componentCenters.forEach(comp => {
    // 1. Culling
    if (comp.x < vb.minX || comp.x > vb.maxX || comp.y < vb.minY || comp.y > vb.maxY) return
    if (!comp.id) return
    if (comp.pinCount <= 1) return

    const label = comp.id
    
    // Determine Position: Prefer Virtual Outline Center
    let posX = comp.x
    let posY = comp.y
    
    const v = virtualMap[comp.id]
    if (v) {
       // Calculate World Center of Virtual Outline
       // Virtual Outline Center is Local (v.center.x, v.center.y) relative to base (v.base_x, v.base_y)
       // And rotated by v.rotation
    ...
    if (!collision) {
      // Fix Text Orientation if flipped
      if (FLIP_VERTICAL) {
         push()
         translate(posX, posY)
         scale(1, -1)
...
function drawLayer_PinLabels(scaleFactor) {
  ...
  if (view.scale <= 5.0) return

  const vb = getVisibleBounds()
  
  // Use smaller text for pins
  const SCREEN_TEXT_SIZE = 9
  const ts = SCREEN_TEXT_SIZE / scaleFactor
...
function touchStarted() {
  if (touches.length === 1) {
    ...
    isPinching = true
    isDragging = false
    touchDistStart = dist(touches[0].x, touches[0].y, touches[1].x, touches[1].y)
...
function touchMoved() {
  if (touches.length === 1 && !isPinching) {
    ...
    // Check threshold against initial start
    if (!hasDragged) {
        const total = dist(touches[0].x, touches[0].y, dragStartX, dragStartY)
    ...
    targetView.offsetX += dx
    targetView.offsetY += dy
    
    // Constrain logic (same as mouseDragged)
    const { scaleFactor } = getTransform()
    const margin = VIEW_MARGIN
    const contentW = (bounds.maxX - bounds.minX) * scaleFactor
    const contentH = (bounds.maxY - bounds.minY) * scaleFactor
    
    if (FLIP_VERTICAL) {
        targetView.offsetX = constrain(targetView.offsetX, -contentW * 2, width + contentW)
        targetView.offsetY = constrain(targetView.offsetY, -contentH * 2, height + contentH)
    } else {
        const minX = PAN_VISIBILITY_MARGIN - contentW
        const maxX = width - 2 * margin - PAN_VISIBILITY_MARGIN
        const minY = PAN_VISIBILITY_MARGIN - contentH
        const maxY = height - 2 * margin - PAN_VISIBILITY_MARGIN
        
        targetView.offsetX = constrain(targetView.offsetX, minX, maxX)
        targetView.offsetY = constrain(targetView.offsetY, minY, maxY)
    }
    ...
    if (touchDistStart > 0) {
      const scaleMult = currentDist / touchDistStart
      const nextScale = constrain(pinchStartScale * scaleMult, 0.1, 50)
      
      // Zoom towards pinch center
      const cx = pinchCenter.x
      const cy = pinchCenter.y
      
      // Calculate world point at pinch center
      ...
      // Recalculate offsets
      view.offsetX = cx - worldX * nextScale
      view.offsetY = cy - worldY * nextScale
...
function touchEnded() {
  if (touches.length === 0) {
    ...
    // If it was a tap (not dragged and not pinched)
    if (!hasDragged && !isPinching) {
      // Use the last touch position or mouseX/Y which p5 updates
      // However, touches[] is empty now. p5 usually updates mouseX/Y on touchEnd too?
      // Let's use mouseX/Y as fallback
...
function mouseReleased() {
  ...
  if (hasDragged) {
    ...
    // When flipped, Y coordinates behave differently for limits
    if (FLIP_VERTICAL) {
        // Adjust limits for flipped view if needed, but actually the pan logic 
        // works on screen space offset.
        // However, the issue might be how the content aligns.
        // Let's relax the limits significantly to prevent getting stuck
        view.offsetX = constrain(view.offsetX, -contentW * 2, width + contentW)
        view.offsetY = constrain(view.offsetY, -contentH * 2, height + contentH)
    } else {
        view.offsetX = constrain(view.offsetX, minX, maxX)
        view.offsetY = constrain(view.offsetY, minY, maxY)
    }
    panVX = 0
    panVY = 0
  }
...
function mouseWheel(event) {
  ...
  // Calculate new offset to keep worldX/Y under mouse
  ...
  // Inverse for ScreenY: ScreenY = ty + sy * (WorldY - baseOffY)
  ...
  // Sync targetView immediately
  targetView.scale = view.scale
  targetView.offsetX = view.offsetX
  targetView.offsetY = view.offsetY
  
  // Stop animation to avoid conflicts
  isAnimating = false
  noLoop()
  updateZoomIndicator()
  redraw()
    ...
    fileInput.addEventListener('change', (e) => {
      ...
      reader.onload = (event) => {
        try {
          ...
          alert('Error al abrir el archivo: Formato inválido')
        }
      }
...
function applyZoomCenter(newScale) {
  ...
  const cx = width / 2
  const cy = height / 2
  // Calculate the world point currently at the center
  const worldX = (cx - view.offsetX) / view.scale
  const worldY = (cy - view.offsetY) / view.scale

  view.scale = nextScale
  
  // Recalculate offsets so that the same world point remains at center
  view.offsetX = cx - worldX * nextScale
  view.offsetY = cy - worldY * nextScale
